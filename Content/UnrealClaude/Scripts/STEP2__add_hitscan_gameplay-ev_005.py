...
def ensure_track(m):
    names = list(al.get_anim_notify_track_names(m))
    if len(names) == 0:
        al.add_anim_notify_track(m, "1", unreal.LinearColor(0.5, 0.5, 0.5, 1.0))
        names = ["1"]
    return str(names[0])
...
for path, t0, t1 in melee:
    try:
        ...
        existing = list(al.get_anim_notify_events(m))
        if existing:
            res.append("SKIP %s already has %d notifies" % (path, len(existing)))
            continue
        n0 = al.add_anim_notify_event(m, track, float(t0), hit_start)
        n1 = al.add_anim_notify_event(m, track, float(t1), hit_end)
...
for path, t0 in ranged:
    try:
        ...
        if existing:
            res.append("SKIP %s already has %d notifies" % (path, len(existing)))
            continue
        n = al.add_anim_notify_event(m, track, float(t0), send_event)
        tagset = "no_notify"
        if n is not None:
            tag, how = make_tag()
            if tag is not None:
