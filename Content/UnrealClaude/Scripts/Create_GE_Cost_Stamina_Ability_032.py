...
dst = FX + "GE_Cost_Stamina_Ability"
eal = unreal.EditorAssetLibrary
if not eal.does_asset_exist(dst):
    if not eal.duplicate_asset(FX + "GE_Cost_Stamina", dst):
        raise RuntimeError("duplicate failed")
cls = unreal.load_object(None, dst + ".GE_Cost_Stamina_Ability_C")
...
if "Value=-33.000000" not in chk or "DecreaseBy20Percent" not in chk or 'AttributeName="Stamina"' not in chk:
    raise RuntimeError(f"verify failed: {chk[:300]}")
saved = eal.save_asset(dst, only_if_is_dirty=False)
print(f"[STEP-OK] GE_Cost_Stamina_Ability: Stamina -33.000000 (curve kept) saved={saved}")
